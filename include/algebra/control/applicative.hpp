...
namespace algebra {
    ...
    struct applicative {
        ...
        template <typename Ff, typename Fn = ValueType<PlainType<Ff>>, typename U = ResultOf<Fn(T)>>
    ...
    struct default_pure {
        ...
        static constexpr F pure(T &&t) noexcept(std::is_nothrow_constructible<F, T &&>::value) {
            return F{std::move(t)};
        }
    };
    ...
    // Overloading `*` as `ap`.
    template <typename Ff, typename F, typename Fn = PlainType<Ff>, typename _F = PlainType<F>,
